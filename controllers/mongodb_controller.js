...
const mongodbController = {
    ...
    // GET LIST TOP NOVEL FROM MONGODB
    getNovelListTopFromMongoDB: async (req, res) => {
        try {
            const perPage = 10;
            console.log('API : getNovelListTopFromMongoDB')
            const novels = await NovelListTop.find({})
                .sort({ rank: 1 }) // Sắp xếp theo thời gian cập nhật mới nhất
                .skip((req.page - 1) * perPage)
                .limit(perPage);
