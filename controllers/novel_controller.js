...
// Hàm bên trong để gọi getNovelInfo
async function getNovelInfoInternal(novel) {
    try {
        console.log('API: getNovelInfo: ' + novel.href.replace("/v1/truyen/", ""))
        ...
        console.log(BASE_URL + '/truyen/' + novel.href.replace("/v1/truyen/", "") + '/')
        ...
        // Chờ cho ít nhất một phần tử chứa danh sách chương xuất hiện trên trang web
        await page.waitForSelector('.col-4.border-bottom-dashed', {timeout: 320000});
        ...
        for(const chapter of novelInfo[0].chapterList) {
            await getChapterContent(chapter, novelInfo[0])
        }
...
/////////////////////////////////////////////////////
async function getChapterContent(chapter, novel)  {
    try {

        console.log('API: getChapterContent: ' + chapter.chapterTitle)
        const nameNovel = chapter.chapterLink.replace(LOCAL_HOST , "").replace("/v1", "")

        const browser = await puppeteer.launch({ headless: "new" }); // Mở trình duyệt ở chế độ ẩn
        const page = await browser.newPage();
        await page.goto(BASE_URL + '/truyen/' + nameNovel);
        console.log(BASE_URL + '/truyen/' + nameNovel)

        await page.waitForSelector('#article', { timeout: 320000 });
        const novelInfo = await page.evaluate((nameNovel, novelParams) => {
            ...
            return {
                ...
                novel: nameNovel.split('/chuong-')[0].replace("/",""),
                chapterNumber: nameNovel.split('/chuong-')[1]
            };
        }, nameNovel, novel);
        await browser.close();
